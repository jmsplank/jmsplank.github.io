...
def create_3d_texture(width, scale, per, pwr):
    ...
    for z in tqdm(coords):
        for y in coords:
            for x in coords:
                v = x * per / width + y * 2 * per / width
                v += pwr * pnoise3(
                    x * scale - half,
                    y * scale - half,
                    z * scale * 0.8 - half,
                ...
                v = 1 - abs(math.sin(v * math.pi)) ** 0.5
...
if __name__ == "__main__":
    ...
    win = pyglet.window.Window(
        ...
        config=pyglet.gl.Config(
            sample_buffers=1, samples=4, double_buffer=True, depth_size=24
        ),
    )

    # noise.randomize()
    create_3d_texture(w // 2, 1 / 48.0, 5, 1.5)
